...
export default function AnalysisPage ()
{
  ...
  useEffect( () =>
  {
    ...
    {
      setIsLoading( true );
      fetchAnalysisReport( repoUrl )
        .then( data => setReport( data ) )
        .catch( err => setError( "Failed to fetch analysis report." ) )
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white">
        <motion.div
          ...
          className="h-12 w-12 border-4 border-t-emerald-400 border-gray-700 rounded-full"
        />
        <p className="mt-4 text-lg text-gray-400">Analyzing repository... this may take a few minutes.</p>
  ...
  const displayData = selectedFile || report; // Show file data if selected, otherwise show project data

  const radarData = [
    { subject: 'Complexity', value: selectedFile ? selectedFile.averageComplexity : report.profile.complexity, fullMark: 10 },
    { subject: 'Quality', value: selectedFile ? selectedFile.averageQuality : report.profile.quality, fullMark: 10 },
    { subject: 'Maintainability', value: selectedFile ? 0 : report.profile.maintainability, fullMark: 10 }, // Example: hide some metrics in file view
    { subject: 'Best Practices', value: selectedFile ? 0 : report.profile.best_practices, fullMark: 10 },
  ];
